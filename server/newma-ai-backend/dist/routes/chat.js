...
chatRouter.post('/', async (req, res) => {
    ...
    const headerKey = authHeader && authHeader.toLowerCase().startsWith('bearer ')
        ...
        : (authHeader || '').trim();
    const finalApiKey = apiKey?.trim() || xApiKeyHeader || headerKey || process.env.OPENAI_API_KEY || process.env.DEEPSEEK_API_KEY || process.env.ANTHROPIC_API_KEY;
    // Debug log headers presence (masked)
    const mask = (k) => k ? '*****' + k.substring(Math.max(0, k.length - 4)) : 'none';
    ...
    try {
        ...
        const sysPrompt = req.body?.systemPrompt || 'You are Newma AI Assistant, a helpful coding assistant integrated into VS Code. Provide clear, concise, and helpful responses in Chinese. Focus on practical solutions and avoid generic templates. If the user asks about code, provide specific examples and explanations.';
...
async function handleSimulatedResponse(prompt, res) {
    console.log('Setting response headers...');
    res.setHeader('Content-Type', 'text/plain; charset=utf-8');
    res.setHeader('Transfer-Encoding', 'chunked');
